...
const {
  HTTP_SUCCESSES, 
  ERROR_TYPES, ERROR_MESSAGES, HTTP_ERRORS,
...
const { 
  NOT_AN_OWNER_OR_A_MANAGER, NOT_A_MANAGER,
  NOT_AN_OWNER, NOT_A_TECHNICIAN,
} = ERROR_TYPES;
const {
  UNAUTHORIZED,
} = HTTP_ERRORS;
...
const expected_tasksList_asTechnician1 = getTasksListAsThisUser(mockTasks, mockUsers[0].user_id);
...
describe('Task REST API Endpoint test', async () => {
  ...
  it('Fetch tasks as a technician', async () => {
  ...
  it('See a task as a manager', async () => {
    const token = await loginNGetJWTToken(manager_loginInfo);
    const { body, status } = await request(server)
  ...
  it('See a task as the task owner technician', async () => {
    const token = await loginNGetJWTToken(technician_1_loginInfo);
    const { body, status } = await request(server)
      .get(`/api/tasks/${expected_task_asManagerOrTechnician.taskId}`)
      .set('Authorization', 'Bearer ' + token);
  ...
  it('See a task as a non-owner technician', async () => {
    ...
    expect(body.message).to.equal(UNAUTHORIZED.getMessage(TASK, expected_task_asManagerOrTechnician.taskId, NOT_AN_OWNER_OR_A_MANAGER).message);
  });


  /** Update a task as a manager or the task owner technician or a non-owner technician */
  it('Update a task as a manager', async () => {
    const token = await loginNGetJWTToken(manager_loginInfo);
    const { body, status } = await request(server)
    ...
    expect(status).to.equal(UNAUTHORIZED.statusCode);
    expect(body).to.have.property('message');
    expect(body.message).to.equal(UNAUTHORIZED.getMessage(TASK, expected_task_asManagerOrTechnician.taskId, NOT_A_TECHNICIAN).message);
  });

  it('Update a task as the task owner technician', async () => {
    const token = await loginNGetJWTToken(technician_1_loginInfo);
    const { body, status } = await request(server)
      .put(`/api/tasks/${expected_task_asManagerOrTechnician.taskId}`)
      .set('Authorization', 'Bearer ' + token)
  ...
  it('Update a task as a non-owner technician', async () => {
    const token = await loginNGetJWTToken(technician_2_loginInfo);
    const { body, status } = await request(server)
      .put(`/api/tasks/${expected_task_asManagerOrTechnician.taskId}`)
      .set('Authorization', 'Bearer ' + token)
    ...
    expect(status).to.equal(UNAUTHORIZED.statusCode);
    expect(body).to.have.property('message');
    expect(body.message).to.equal(UNAUTHORIZED.getMessage(TASK, expected_task_asManagerOrTechnician.taskId, NOT_AN_OWNER).message);
  });


  /** Perform a task as a manager or the task owner technician or a non-owner technician */
  it('Perform a task as a manager', async () => {
    const token = await loginNGetJWTToken(manager_loginInfo);
    const { body, status } = await request(server)
      .put(`/api/tasks/perform/${expected_task_asManagerOrTechnician.taskId}`)
      .set('Authorization', 'Bearer ' + token);
    expect(status).to.equal(UNAUTHORIZED.statusCode);
    expect(body).to.have.property('message');
    expect(body.message).to.equal(UNAUTHORIZED.getMessage(TASK,expected_task_asManagerOrTechnician.taskId, NOT_A_TECHNICIAN).message);
  ...
  it('Perform a task as the task owner technician', async () => {
    const token = await loginNGetJWTToken(technician_1_loginInfo);
    const { body, status } = await request(server)
      .put(`/api/tasks/perform/${expected_task_asManagerOrTechnician.taskId}`)
      .set('Authorization', 'Bearer ' + token);
  ...
  it('Perform a task as a non-owner technician', async () => {
    const token = await loginNGetJWTToken(technician_2_loginInfo);
    const { body, status } = await request(server)
      .put(`/api/tasks/perform/${expected_task_asManagerOrTechnician.taskId}`)
      .set('Authorization', 'Bearer ' + token);
  ...
  it('Create a new task as a manager', async () => {
    const token = await loginNGetJWTToken(manager_loginInfo);
    const { body, status } = await request(server)
      .post('/api/tasks')
      .set('Authorization', 'Bearer ' + token)
  ...
  it('Create a new task as a technician', async () => {
    const token = await loginNGetJWTToken(technician_1_loginInfo);
    const { body, status } = await request(server)
      .post('/api/tasks')
      .set('Authorization', 'Bearer ' + token)
  ...
  it('Delete a task as a manager', async () => {
    const token = await loginNGetJWTToken(manager_loginInfo);
    const { body, status } = await request(server)
  ...
  it('Delete a task as a techinician', async () => {
    const token = await loginNGetJWTToken(technician_1_loginInfo);
    const { body, status } = await request(server)
      .delete(`/api/tasks/${expected_task_asManagerOrTechnician.taskId}`)
      .set('Authorization', 'Bearer ' + token);
    expect(status).to.equal(UNAUTHORIZED.statusCode);
    expect(body).to.have.property('message');
    expect(body.message).to.equal(UNAUTHORIZED.getMessage(TASK, expected_task_asManagerOrTechnician.taskId, NOT_A_MANAGER).message);
  });
});