...
const ERROR_MESSAGES = {
    ...
    [ERROR_TYPES.NOT_AN_OWNER]: {
        getMessage: () =>
            `You are not the owner of this task to update this task.`,
    },
    [ERROR_TYPES.NOT_AN_OWNER_OR_A_MANAGER]: {
        getMessage: () =>
            `You are neither the owner of this task nor a manager to get access to this task.`,
    },
    [ERROR_TYPES.NOT_A_MANAGER]: {
        getMessage: () =>
            `You are not a manager to have access to view or delete tasks.`,
    },
    [ERROR_TYPES.NOT_A_TECHNICIAN]: {
        getMessage: () =>
            `You are not a technician to create, update or perform tasks.`,
    },
    ...
    [ERROR_TYPES.LIST_ALL_FAIL]: {
        getMessage: (resourceType) =>
            `Error retrieving a list of ${resourceType}.`,
    },
    [ERROR_TYPES.UPDATE_FAIL]: {
        getMessage: (resourceType, id) =>
            `Cannot update ${resourceType} with ${id}. Maybe ${resourceType} was not found or req.body is empty!`,
    },
    [ERROR_TYPES.DELETE_FAIL]: {
        getMessage: (resourceType, id) =>
            `Cannot delete ${resourceType} with ${id}. Maybe ${resourceType} was not found!`,
    },
};
const HTTP_ERRORS = {
    /** 400s */
    ...
    NOT_FOUND: {
        statusCode: 404,
        getMessage: (resourceType, id) => ({
            message: `Cannot find ${resourceType} with ${id}.`,
        }),
    },
    /** 500s */
    INTERNAL_SERVER: {
        statusCode: 500,
        getMessage: (resourceType, id) => ({
            message: `Error retrieving ${resourceType} with ${id}.`,
        }),
    },
};
/** ------ */
